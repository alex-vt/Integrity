...
package com.alexvt.integrity.lib.android.ui.types
...
abstract class DataTypeActivity : ThemedActivity() {
    ...
    private fun openFolderLocationList(selectMode: Boolean) {
        ...
        intent.component = ComponentName("com.alexvt.integrity",
                "com.alexvt.integrity.android.ui.destinations.DestinationsActivity") // todo resolve
    ...
    private fun updateFolderLocationSelectionInViews(folderLocations: List<FolderLocation>) {
        binding.tvArchiveLocations.text = folderLocations.joinToString(separator = ", ") {
            destinationNameRepository.getDestinationName(it.javaClass)
        }
    }
    ...
    private fun openTagList(selectMode: Boolean) {
        val intent = Intent()
        intent.component = ComponentName("com.alexvt.integrity",
                "com.alexvt.integrity.android.ui.tags.TagsActivity") // todo resolve
        IntentUtil.putSelectMode(intent, selectMode)
        IntentUtil.putSnapshot(intent, TypeSpecificMetadataConverter.fromTypeSpecificMetadata(this, snapshot))
        startActivityForResult(intent, 0)
    }
